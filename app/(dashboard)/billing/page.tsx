...
export default async function BillingPage({ searchParams }: BillingPageProps) {
  ...
  if (!user) {
    // Preserve the chosen plan through login so checkout resumes afterwards.
    const next = requestedPlan
      ? `?next=${encodeURIComponent(`/billing?plan=${requestedPlan}`)}`
      : "";
    redirect(`/login${next}`);
  }
  ...
  // When arriving with a pre-selected plan (e.g. from the landing page), send
  // the user straight to Stripe Checkout instead of making them click again.
  const autoCheckout =
  ...
  // For active subscribers, load live subscription state so we can show the
  // real current plan, renewal date, and offer in-app plan changes + cancel.
